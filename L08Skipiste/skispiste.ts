namespace L08_Skipiste {
    ...
    let golden: number = 0.70; //Goldenen Schnitt
    ...
    function handleLoad(_event: Event): void {
        ...
        let slopeWidthBack: number = 400;
        let slopeWidthFront: number = 1400;
        let treesOffsetBack: number = 150;
        let treesOffsetFront: number = 5;
        
        //Positionen
        let posMountains: Vector = { x: 0, y: 600 };
        let posSlope: Vector = { x: 1000, y: 300};
        let posTreesStart: Vector = { x: posSlope.x - slopeWidthBack / 2 - treesOffsetBack, y: horizon };
        let posTreesEnd: Vector = { x: crc2.canvas.width / 2 - slopeWidthFront / 2 - treesOffsetFront, y: crc2.canvas.height };
    ...
    function drawTree(): void {
        console.log("Tree");
        let nBranches: number = 200; //Anzahl Ast
        let maxRadius: number = 60; //Radius größe Ast
        ...
        do {
            let y: number = Math.random() * 350; //What???????????
            ...
            let color: string = "HSLA(" + colorAngle + ", 60%, 10%, 0.5)";
    ...
    function drawBackground(): void {
        ...
        let gradient: CanvasGradient = crc2.createLinearGradient(0, 0, 0, crc2.canvas.height); //Variable vom Typ Gradient vom Typ CanvasGradient, Gradient von oben nach unten (entlang einer Linie)
    ...
    function drawCloud(_position: Vector, _size: Vector): void { //Position und Size vom Typ Vektor
        ...
        for (let drawn: number = 0; drawn < nParticles; drawn++) { //zeichnen solange die anzahl der gezeichneten Partikel kleiner als Partikel die wir möchten
            crc2.save();
            let x: number = (Math.random() - 0.5) * _size.x;
            let y: number = - (Math.random() * _size.y); //Mit Ausdehnung in der Vertikalen multipliziert
            crc2.translate(x, y);
            crc2.fill(particle);
            crc2.restore();
        }
    ...
    function drawMountains(_position: Vector, _min: number, _max: number, _colorLow: string, _colorHigh: string): void {
        ...
        do {
            ...
            let y: number = -_min - Math.random() * (_max - _min); //y definiert, Zufallszahl
        ...
        let gradient: CanvasGradient = crc2.createLinearGradient(0, 0, 0, -_max); //Anfang beim Fuß des Berges bis zur maximal Höhe
    ...
    function drawSnowslope(_position: Vector, _widthBack: number, _widthFront: number): void { //Vektor für Position horizontal, Breite hinten und vorne
